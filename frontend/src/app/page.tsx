...
export default function Home() {
  ...
  const fetchSnippetFromBackend = async (lang: Language) => {
    try {
      const response = await fetch(`http://localhost:5000/api/snippets/random?language=${lang}`);
      if (!response.ok) {
      ...
      // Fallback to local snippets if backend is down
      return getRandomSnippet(lang);
    }
  };
  // Initialize snippet on client side only
  useEffect(() => {
    fetchSnippetFromBackend(language).then(setSnippet);
}, []);
  ...
  const handleNewTest = useCallback(() => {
    ...
    setTimeout(async () => {  // Make this async
        if (stats.totalChars > 0) {
            ...
            const newSnippet = await fetchSnippetFromBackend(language);  // Use backend
